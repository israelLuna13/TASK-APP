...
import { Project, TeamMember, TeamMemberForm, teamMembersShema } from '../types'
...
export async function getProjectTeam(projectId:Project['_id']){
    try {
        const url = `/projects/${projectId}/team`
      //  const {data} = await api<teamMembersShema>(url)  
        const {data} = await api(url)        
        //other way the validate the response of our api
        const response = teamMembersShema.safeParse(data)
        if(response.success){
